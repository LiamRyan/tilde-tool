// Command Line Virtual Desktop by MScholtes
...
namespace VirtualDesktopApi
{
   ...
   [ComImport]
   [Guid("cd403e52-deed-4c13-b437-b98380f2b1e8") /* replace at runtime */]
   [InterfaceType(ComInterfaceType.InterfaceIsIUnknown)]
   ...
   public static class VirtualDesktopManager
	...
	public partial class VirtualDesktop
	{
      ...
      internal static void RebuildDictionary()
      {
         ...
         int count = VirtualDesktopManager.VirtualDesktopManagerInternal.GetCount(IntPtr.Zero);
		...
		{ // returns current desktop
			get { return FromInterface(VirtualDesktopManager.VirtualDesktopManagerInternal.GetCurrentDesktop(IntPtr.Zero)); }
		}
		...
		{ // return desktop object to desktop on which window <hWnd> is displayed
			...
			if ((id.CompareTo(AppOnAllDesktops) == 0) || (id.CompareTo(WindowOnAllDesktops) == 0))
				return FromInterface(VirtualDesktopManager.VirtualDesktopManagerInternal.GetCurrentDesktop(IntPtr.Zero));
			else
      ...
      public static string DesktopNameFromDesktop(VirtualDesktop desktop)
		...
		{ // get index of desktop with partial name, return -1 if no desktop found
			...
			for (int i = 0; i < VirtualDesktopManager.VirtualDesktopManagerInternal.GetCount(IntPtr.Zero); i++)
		...
		{ // create a new desktop
			return FromInterface(VirtualDesktopManager.VirtualDesktopManagerInternal.CreateDesktop(IntPtr.Zero));
		}
		...
		public int Index { get {
            ...
            IObjectArray desktops;
            VirtualDesktopManager.VirtualDesktopManagerInternal.GetDesktops(IntPtr.Zero, out desktops);
            object objdesktop;
            for (int i = 0; i < VirtualDesktopManager.VirtualDesktopManagerInternal.GetCount(IntPtr.Zero); i++)
            {
      ...
      public void Remove(VirtualDesktop fallback = null)
		...
		{ // remove all desktops but visible
			VirtualDesktopManager.VirtualDesktopManagerInternal.SetDesktopIsPerMonitor(true);
		}

		public void Move(int index)
		{ // move current desktop to desktop in index (-> index = 0..Count-1)
			VirtualDesktopManager.VirtualDesktopManagerInternal.MoveDesktop(ivd, IntPtr.Zero, index);
		}
		...
		{ // return true if this desktop is the current displayed one
			get { return object.ReferenceEquals(ivd, VirtualDesktopManager.VirtualDesktopManagerInternal.GetCurrentDesktop(IntPtr.Zero)); }
		}

		public void MakeVisible()
		{ // make this desktop visible
			VirtualDesktopManager.VirtualDesktopManagerInternal.SwitchDesktop(IntPtr.Zero, ivd);
		}
